    ...
    public class UserRepository : IUserRepository
    {
        ...
        public async Task<User> GetUserById(string id)
        {
            var pipeline = new BsonDocument[]
            {
                ...
                //join bảng userfollow lấy followedid
                new BsonDocument("$lookup", new BsonDocument
                {
                    { "from", "UserFollowCollection" },
                    { "localField", "_id" },
                    { "foreignField", "FollowedId" },
                    { "as", "followers" }
                }),
           
                //join bảng userfollow lấy followerid
                new BsonDocument("$lookup", new BsonDocument
                {
                    { "from", "UserFollowCollection" },
                    { "localField", "_id" },
                ...
                new BsonDocument("$project", new BsonDocument
                {
                    ...
                    { "Description", 1 },
                    { "followersCount",
                        new BsonDocument("$size", "$followers") // đếm số lượng follow
                     },
                    { "followingCount",
                        new BsonDocument("$size", "$following")
                    },
                })
            };
